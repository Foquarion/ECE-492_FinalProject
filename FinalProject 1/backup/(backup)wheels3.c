...
unsigned char current_car_left_on_line ;    // ADC Right = Physical Left
unsigned char current_car_right_on_line;    // ADC Left = Physical Right
...
void Line_Follow_Process(void) {
    ...
    unsigned char car_left_on_line = adc_right_on_line;    // ADC Right = Physical Left
    unsigned char car_right_on_line = adc_left_on_line;    // ADC Left = Physical Right
    
    unsigned char car_left_crossed = adc_right_crossed;    // ADC Right = Physical Left
    unsigned char car_right_crossed = adc_left_crossed;    // ADC Left = Physical Right
    ...
    case PART1_ALIGN_TURN:
        ...
        if (car_right_detected_first) {
            // Right detected first = car veering RIGHT = turn RIGHT to correct
            PWM_TURN_RIGHT();
        }
        else if (car_left_detected_first) {
            // Left detected first = car veering LEFT = turn LEFT to correct
            PWM_TURN_LEFT();
        }
        else {
    ...
    case LINE_FOLLOWING:
        ...
        current_car_left_on_line = current_adc_right_on_line;    // ADC Right = Physical Left
        current_car_right_on_line = current_adc_left_on_line;    // ADC Left = Physical Right
        
        // Track which side has darkest reading (for recovery guidance)
        last_adc_left_value = ADC_Left_Detect;    // Physical RIGHT sensor
        last_adc_right_value = ADC_Right_Detect;  // Physical LEFT sensor
...
void Line_Follow_Proportional(void) {
     ...
     * 
     * Remember: ADC_Left_Detect is physically RIGHT side
     *           ADC_Right_Detect is physically LEFT side
     */

    // Calculate how far each sensor is from threshold
    int adc_left_error = ADC_Left_Detect - LEFT_THRESHOLD;   // Physical RIGHT sensor
    int adc_right_error = ADC_Right_Detect - RIGHT_THRESHOLD; // Physical LEFT sensor

    // Map to physical car sides
    int car_right_error = adc_left_error;   // Physical RIGHT
    int car_left_error = adc_right_error;   // Physical LEFT
